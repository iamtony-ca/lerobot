...
* **이론적 분석:** Phase 2의 추가 데이터가 모델의 시각적 어텐션을 복구했습니다. 목적지의 위치가 무작위로 변함에 따라, 로봇은 더 이상 관절 좌표의 암기만으로는 훈련 오차를 좁힐 수 없게 되었습니다. 또한, 100번의 다채로운 접근 궤적이 누적되면서 손목 카메라 영상에 나타나는 '객체 픽셀 크기의 비선형적 팽창'을 Transformer가 거리 데이터로 치환 해석 했거나 혹은 Data Scaleup으로 인한 Motion 자체가 일반화 혹은 Overfit 됐다고 해석 할 수 있습니다. 이는 데이터 스케일링이 RGB 모델의 3D 공간 지각력에 미치는 영향을 간접적으로 해석할 수 있습니다.
* **시사점:** 데이터셋을 구성할 때 모든 변수(Pick 위치, Place 위치)를 처음부터 어려운 난이도로 무작위화하면 모델이 학습 방향을 잃고 발산(Underfitting)할 가능성이 있어서, 본 테스트처럼 전체 데이터의 절반은 단일 변수(Pick 위치만 변경)로 구성하여 '파지(Grasp)'라는 기본 동작의 특징 추출(Feature Extraction)을 하게 하고, 나머지 절반에 다중 변수(모두 변경)를 섞어 모델이 적절히 수렵할 수 있도록 해줌. 데이터 수집 시, 로봇이 인지하고 상호작용해야 하는 모든 객체(목표물, 장애물 등)에 물리적 변동성(Randomization)을 부여해야만 모델이 올바른 시각적 인과관계를 학습하거나, Task 수행 능력이 향상됨.

### Case 4: Closed-Loop 제어 기반의 Pick 재시도 성공 (100 Ep 모델)

* **상황:** 초기 파지에 실패하여 빈 그리퍼로 닫혔으나, 그대로 허공에 Place하지 않고 다시 그리퍼를 열어 위치를 재조정한 뒤 인형을 완벽하게 파지하여 성공함. (※ 전체 100개 데이터 중 복구 시연이 포함된 데이터는 10개 미만으로 극소수였음)
* **이론적 분석:** 복구 시연 데이터가 극소수였음에도 재시도에 성공한 것은 정책(Policy)이 특정 복구 궤적을 암기해서가 아니라, **상태의 중첩(State Overlap)**을 활용한 완벽한 **반응형 폐루프 제어(Reactive Closed-Loop Control)**로 작동했기 때문입니다.
* 초기 파지 실패 직후의 카메라 이미지($s_t$)는 "비어 있는 그리퍼와 테이블 위의 인형"입니다. 이 시각적 상태는 로봇이 이전에 90번 이상 성공적으로 학습했던 **'접근 단계(Approach Phase)의 초기 상태'와 시각적으로 매우 유사**합니다.
* 마르코프 결정 과정(MDP)에 따라, 모델은 "내가 방금 실패했다"는 과거의 맥락(History)에 얽매이지 않습니다. 오직 현재의 시각적 상태($s_t$)만을 평가하여, "지금 상태에서는 인형을 향해 손을 뻗는 행동($a_t$)이 정답이다"라고 매칭한 것입니다. 10개 미만의 소수 복구 데이터는 모델이 실패 상태에서 OOD(Out-Of-Distribution)로 빠져 프리징(Freezing)되는 것을 막아주는 역할만을 수행했을 것으로 보임.


* **시사점:** 데이터 수집 시, 로봇이 겪을 수 있는 모든 실패 시나리오와 복구 궤적을 수백 번씩 의도적으로 촬영할 필요가 없습니다. Closed-Loop 제어가 보장된다면, **전체 데이터의 10% 미만의 자연스러운 인간의 실수 및 복구 데이터(Sub-optimal Demo)만 포함되어도 모델은 실패 상태를 기존의 정상 상태로 치환하여 스스로 복구하는 반응형 정책(Reactive Policy)이 가진 일반화 능력**을 갖게 됨을 확인했습니다.
...
* **시사점:** 모델 내부의 사전 지식 충돌을 최소화하려면 비동기 추론 서버의 파라미터 최적화가 영향을 줄 수 있음. '--chunk_size_threshold'는 높이고, '--actions_per_chunk'는 낮춰서 과거 청크의 유효 수명을 줄이고, 가장 최근 시각적 프레임에 상대적으로 큰 weight를 부여하여 제어 로직을 튜닝 할 수 있음. 또한 OOD 상황을 방지하기 위해서 Dataset의 Quality 향상과 Scaleup을 통해 개선할 수 있을 것으로 보임.
