...
현재 적용된 ACT 알고리즘은 단순한(Simple) 태스크에 대해서는 준수한 성능을 보장합니다. 하지만 비전 백본(Vision Backbone)으로 CNN 기반의 ResNet을 사용하고 있어, 시각적 특징 추출의 스케일업과 복잡한 환경 이해도에 태생적인 한계가 관찰됩니다. 향후 다양한 변수가 존재하는 복합적인(Complex) 태스크를 수행하기 위해서는, ViT(Vision Transformer)의 강력한 시각 인지와 거대 언어 모델이 결합된 **VLA (Vision-Language-Action) 모델** (예: Groot N1.X 등)로의 전환이 필수적일 것으로 판단됩니다.
