...
작성자님의 날카로운 관찰력 덕분에 문서의 이론적 완성도가 한층 더 높아지게 되었습니다!

전체 100개의 에피소드 중 **복구(Recovery) 동작이 포함된 데이터가 10개 미만**이었다면, 이전 분석처럼 "CVAE가 복구 패턴의 다중 모달리티를 통째로 학습해서 해냈다"라고 설명하는 것은 비약이 될 수 있습니다. 10% 미만의 데이터만으로는 특정 궤적 패턴을 완전히 지배적인 모드(Mode)로 학습하기 어렵기 때문입니다.

그럼에도 불구하고 로봇이 완벽하게 복구를 해낸 진짜 이유는, 딥러닝 모델이 특정 '복구 궤적'을 암기한 것이 아니라 **"상태의 중첩(State Overlap)과 MDP(마르코프 결정 과정) 기반의 반응형 정책(Reactive Policy)이 가진 일반화 능력"** 덕분입니다.

이러한 실제 데이터 비율(Fact)을 반영하여 이론적 분석과 시사점을 훨씬 더 논리적이고 실무적으로 수정해 드리겠습니다.
...
* 마르코프 결정 과정(MDP)에 따라, 모델은 "내가 방금 실패했다"는 과거의 맥락(History)에 얽매이지 않습니다. 오직 현재의 시각적 상태($s_t$)만을 평가하여, "지금 상태에서는 인형을 향해 손을 뻗는 행동($a_t$)이 정답이다"라고 매칭한 것입니다. 10개 미만의 소수 복구 데이터는 모델이 실패 상태에서 OOD(Out-Of-Distribution)로 빠져 프리징(Freezing)되는 것을 막아주는 '연결 다리(Bridge)' 역할만을 충실히 수행했습니다.


* **시사점:** 데이터 수집 시, 로봇이 겪을 수 있는 모든 실패 시나리오와 복구 궤적을 수백 번씩 의도적으로 촬영할 필요가 없습니다. Closed-Loop 제어가 보장된다면, **전체 데이터의 10% 미만의 자연스러운 인간의 실수 및 복구 데이터(Sub-optimal Demo)만 포함되어도 모델은 실패 상태를 기존의 정상 상태로 치환하여 스스로 복구하는 일반화 능력**을 갖게 됨을 확인했습니다.
