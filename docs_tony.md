...
* **시사점:** 이 문제를 근본적으로 해결하기 위해서는 ZED X 등 스테레오 카메라를 활용하여 $Z$축 데이터를 명시적으로 주입(RGB-D)하는 것이 이상적입니다. 단, ACT 네트워크 구조 변경과 초근접 시 발생하는 Depth 센서 노이즈 필터링 등 하드웨어-소프트웨어 통합 레이어의 튜닝이 동반되어야 합니다.
...
* **시사점:** 초기(Phase 1)에는 쉬운 과제(단일 변수)를 주어 파지(Grasp)라는 기본 동작의 신뢰도를 높이고, 점진적으로 난이도(다중 변수)를 올리는 **커리큘럼 학습(Curriculum Learning)** 전략이 파국적 망각(Catastrophic Forgetting)을 막고 모델을 가장 안정적으로 수렴시킴을 확인했습니다.
...
* **시사점:** 모델 내부의 사전 지식 충돌을 최소화하려면 비동기 추론 서버의 파라미터 최적화가 중요합니다. `--chunk_size_threshold` 값을 조정하여 과거 청크의 유효 수명을 줄이고, 가장 최근 시각적 프레임이 지배적인 권한을 갖도록 제어 로직을 튜닝해야 합니다.
...
**2. 모바일 매니퓰레이터(Mobile Manipulator) 통합을 위한 핵심 기반 마련**
실내 주행 로봇(AMR)이 내비게이션(예: Nav2)을 통해 목표 지점에 도달할 때, 슬립(Slip)이나 센서 노이즈로 인해 필연적으로 **정차 위치 오차(Tolerance)**가 발생합니다. 본 테스트에서 검증된 ACT 모델의 '시각적 적응력(Adaptive Pick & Place)'과 'OOD 복구 능력'은 이러한 AMR의 주행 오차를 매니퓰레이터가 스스로 보상(Compensation)할 수 있음을 의미합니다. 즉, 완벽한 밀리미터 단위의 주행 정밀도에 집착하지 않아도 유연하게 태스크를 완수할 수 있는 자율 시스템 구축의 초석을 다졌습니다.

**3. 데이터 중심(Data-Centric) 로보틱스의 방법론적 기준 제시**
