...
* **VLA 모델로의 진화 필요성:** ACT 알고리즘은 Simple Task에서 훌륭한 성능을 보이나, Vision Backbone으로 CNN(ResNet)을 사용하고 있어 복잡한 환경 이해도와 시각적 특징 추출의 스케일업에 태생적인 한계가 관찰됩니다. 향후 다변수 Complex Task를 수행하기 위해서는 ViT(Vision Transformer)의 강력한 시각 인지와 거대 언어 모델이 결합된 **VLA(Vision-Language-Action) 모델**(예: Groot N1.X)로의 전환이 필수적일 것으로 판단됩니다.
* **데이터 구축 파이프라인의 현실적 병목:** Data Quality를 결정짓는 시연자(Expert)의 숙련도와 Leader Device의 사용 편의성이 전체 파이프라인의 핵심 병목(Bottleneck)임을 체감했습니다. 향후 고도화된 AI를 위해서는 데이터 수집 및 액션 데이터 생성의 자동화/편의성 개선이 매우 중요해질 것입니다. 이는 최근 학계에서 액션 데이터 생성을 자동화하려는 연구들이 대두되는 맥락과 일치합니다.
* **컴퓨팅 인프라의 제약:** 현재 환경(VRAM 8GB GPU)에서는 Data Processing 규모와 모델 최적화를 한계치까지 끌어올리는 데 제약이 있었습니다. 향후 인프라 확충 시 배치 사이즈 증가 및 고해상도 입력 처리를 통한 획기적인 성능 향상이 기대됩니다.
